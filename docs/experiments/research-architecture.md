...
| W9 | Rehearse the replacement candidate / maintainer + operations owner | W8; accepted deployment manifest | **In progress.** The isolated candidate is deployed, and the matched compatibility comparison passed all declared journeys in three runs per arm. Migration and rollback pass with PostgreSQL authority and a retained Qdrant fallback. A real-client audit exposed and repaired streaming search, candidate Parse wiring, semantic timeout, staged Parse metadata, and the missing public staged-upload reservation. The final runtime change restarted the stability window at `2026-09-19T19:00:53.139536Z`; checkpoint 0 passed all 12 declared operations. Complete checkpoints at 72 and 168 elapsed hours, then publish the evidence-based architecture/adoption decision. | Earliest completion `2026-09-26T19:00:53.139536Z` plus active review |
