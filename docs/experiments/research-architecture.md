...
| W10 | [Adaptive research policy hardening](https://github.com/magnus919/groktocrawl-x/milestone/9) | [#314 bounded adaptive-query policies](https://github.com/magnus919/groktocrawl-x/issues/314) | In progress. The research brief, 12-case challenge stratum, 24-case W8 anchor, five policies, replacement gates, source ledger, and sensitivity plan are refrozen. Methodology audits excluded one launch that assessed stopping only after dispatch and another whose completed record retained a nonterminal `continue` label. The corrected full policy now assesses the first follow-up before dispatching the second and requires an explicit terminal reason. Live smokes exercised both the 72.153-second no-gain stop and the 50.639-second planner-complete path. The valid 180-trial challenge launch follows this freeze; the 360-trial anchor, adjudication, synthesis, and ADR decision follow. |
