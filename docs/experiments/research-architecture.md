...
| W1 | Architecture decision pack and baseline / architecture + evaluation owner | W0 identity; execution lanes needed for measured baseline | Review D1–D3/D6 contracts, frame D4/D5 alternatives, accept D7 experiment protocol; pin baseline behavior and decisions with measurable unresolved questions. The original packet, isolation review, incumbent run, and [primary semantic review](enterprise-evaluation/w1-incumbent-semantic-review-2026-09-09.md) are complete. Proposed bounds and execution controls are pinned. Candidate B now has strict schema generation, one ordered semantic review batch, and deterministic render auditing; live probes removed the invalid-label failure but exposed local-backend latency/timeouts and ended with gateway HTTP 502. Candidate B remains unfrozen until a complete terminal run. Primary review exposed the original packet to the implementation agent, so a fresh comparison packet must be curated only after the candidate freezes. Human adjudication, replacement packet, and comparison authorization remain open. | 3–5 days |
...
| W4 | Equivalent workflow backends / research + evaluation owner | W2, D7; W3 for persistence cases | **Current-workflow comparison complete; future substrate open:** the optional LangGraph adapter and two 30-repetition-per-workload paired series produced 720 records with zero conformance failures. The imperative controller remains the bounded-workflow reference. Issue #245 evaluates future-facing research capabilities before ADR-0073 decider review | 3–5 days plus future-capability experiment |
