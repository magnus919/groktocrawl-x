...
| W9 | Rehearse the replacement candidate / maintainer + operations owner | W8; accepted deployment manifest | **In progress.** The isolated candidate is frozen and deployed, and the matched compatibility comparison passed all declared journeys in three runs per arm. Rehearse migration and rollback, complete the bounded side-by-side pilot, and publish the evidence-based architecture/adoption decision | 7-day observation window plus active setup and review |
