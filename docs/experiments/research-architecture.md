...
| W11 | [SlopSearX research substrate integration](https://github.com/magnus919/groktocrawl-x/milestone/10) | [#318](https://github.com/magnus919/groktocrawl-x/issues/318) | Active; the scored comparison waits for W10. The installed SlopSearX 0.5 image is the pinned W11 baseline. Its opt-in adaptive continuation, entity grouping, saved-search reports, retrieval receipts and manifests, staged search, and dependency dossiers are split into isolated grant-specific arms. HTTP compatibility and all nine grant boundaries passed. All 33 deterministic cases now have exact proof from 116 local outcomes and three isolated-Valkey outcomes. Live provenance, entity projection, receipt-interruption, process-killed capture recovery, and saved-search lifecycle gates passed. Controlled saved-search change detection remains deterministic evidence; the dependency-dossier live slice remains open. GroktoCrawl owns user intent, evidence gaps, synthesis, and publication; SlopSearX owns bounded retrieval execution, immutable snapshots, provenance, and attempt accounting. Ordinary SearXNG behavior remains unchanged. |
