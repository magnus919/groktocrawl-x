...
| W10 | [Adaptive research policy hardening](https://github.com/magnus919/groktocrawl-x/milestone/9) | [#314 bounded adaptive-query policies](https://github.com/magnus919/groktocrawl-x/issues/314) | In progress. The refrozen 180-trial challenge and 360-trial W8 anchor are complete under the same frozen policy and environment: all 540 trial identities have valid completed records and both independent validators pass. Fifteen original anchor transport failures remain preserved in the missing-data audit; a resumable recovery pass repaired those identities without discarding the failed attempts. The summary and blinded adjudication packet are complete. LiteLLM removed the Luna route after 985 valid judgments; that incomplete run is preserved but excluded from scoring. Eleven General judgments made through the Hermes agent CLI are likewise retained only as transport diagnostics because the CLI exhausted its run budget during model reasoning. A clean scored run now sends fresh OpenAI-compatible requests directly to the same LiteLLM `general` alias over ten disjoint lanes, with model and transport recorded separately. The remaining sequence is adjudication completion, sensitivity and accounting analysis, final synthesis, ADR decision, CI, and merge. |
| W11 | [SlopSearX research substrate integration](https://github.com/magnus919/groktocrawl-x/milestone/10) | [#318](https://github.com/magnus919/groktocrawl-x/issues/318), [PR #319](https://github.com/magnus919/groktocrawl-x/pull/319) | Active behind W10. SlopSearX 0.5.0 is pinned by source and image digest. HTTP compatibility, all nine grant boundaries, no-dispatch engine-scope equivalence, all 40 deterministic cases, isolated-Valkey repetitions, and the live provenance, entity, recovery, saved-search, dependency-dossier, and composition slices passed. The scored flat-search versus recorded-continuation comparison has a predeclared analysis plan, W10-bound work order, fail-closed freeze gate, matched result limit, and enforced evidence ceiling. GroktoCrawl retains ownership of intent, gap decisions, synthesis, and publication. Scored measurement remains prohibited until W10 supplies the selected control policy. |
